...
public:
            ...
            {
                {
                    ...
                    ++currow;
                    {
                        ...
                        encLayout->addWidget(v_resolution_ = new QComboBox(gp), currow, curcol++);
                        v_resolution_->setEditable(true);
                    }
                    ++currow;
                    {
                        int curcol = 0;
    ...
    {
        v_resolution_->clear();
        v_resolution_->addItem(obs_module_text("SameAsOBSNow"), "");
        v_resolution_->addItem(QStringLiteral("1080x1920 (9:16 Vertical Full HD - TikTok/Insta/Shorts)"), "1080x1920");
        v_resolution_->addItem(QStringLiteral("720x1280 (9:16 Vertical HD)"), "720x1280");
        v_resolution_->addItem(QStringLiteral("1080x1080 (1:1 Quadrado / Square)"), "1080x1080");
        v_resolution_->addItem(QStringLiteral("1920x1080 (16:9 Full HD)"), "1920x1080");
        v_resolution_->addItem(QStringLiteral("1280x720 (16:9 HD)"), "1280x720");
        v_resolution_->addItem(QStringLiteral("2560x1440 (16:9 2K QHD)"), "2560x1440");
        v_resolution_->addItem(QStringLiteral("3840x2160 (16:9 4K UHD)"), "3840x2160");
    }
    ...
    void SaveVideoConfig() {
        ...
        QString resStr = v_resolution_->currentData().toString();
        if (resStr.isEmpty()) {
            resStr = v_resolution_->currentText().trimmed();
            if (resStr == obs_module_text("SameAsOBSNow"))
                resStr = "";
        }
        if (!resStr.isEmpty())
            it->resolution = resStr.toUtf8().constData();
        else
            it->resolution.reset();
    ...
    void LoadVideoConfig(IdOrVideoConfig idOrConfig) {
        ...
        {
            QString currentRes = QString::fromUtf8(config.resolution.value_or(""));
            int rIdx = v_resolution_->findData(currentRes);
            if (rIdx >= 0) {
                v_resolution_->setCurrentIndex(rIdx);
            } else if (!currentRes.isEmpty()) {
                v_resolution_->setEditText(currentRes);
            } else {
                v_resolution_->setCurrentIndex(0);
            }
        }
