...
class PushWidgetImpl : public PushWidget, public IOBSOutputEventHanlder
{
    ...
    bool PrepareEncoderSource() {
        ...
        if (!using_main_video_encoder_) {
            ...
            bool hasScene = (videoConfig && videoConfig->outputScene.has_value() && !videoConfig->outputScene->empty());
            bool hasResolution = (videoConfig && videoConfig->resolution.has_value() && !videoConfig->resolution->empty());

            if (!hasScene && !hasResolution) {
                obs_encoder_set_video(venc, obs_get_video());
            } else {
                obs_source_t* scene = nullptr;
                if (hasScene) {
                    scene = obs_get_source_by_name(videoConfig->outputScene->c_str());
                }
                if (!scene) {
                ...
                if (hasResolution) {
                    auto res = ParseResolution(videoConfig->resolution);
                    if (res.has_value()) {
                        auto [w, h] = *res;
                        ovi.base_width = static_cast<uint32_t>(w);
                        ovi.base_height = static_cast<uint32_t>(h);
                        ovi.output_width = static_cast<uint32_t>(w);
                        ovi.output_height = static_cast<uint32_t>(h);
                    }
                }

                if (videoConfig && videoConfig->fpsDenumerator > 1) {
                    ovi.fps_den *= videoConfig->fpsDenumerator;
                }

                auto scene_video = obs_view_add2(scene_view_, &ovi);
                if (!scene_video) {
                    blog(LOG_ERROR, TAG "Failed to create secondary video mix with obs_view_add2.");
                    return false;
                }
    ...
    void UpdateStreamStatus() {
        ...
        if (interval > 0)
        {
            ...
            auto dropped_frames = obs_output_get_frames_dropped(output_);
            double drop_percent = new_frames > 0 ? (100.0 * dropped_frames / new_frames) : 0.0;
            char strDropped[64] = { 0 };
            snprintf(strDropped, sizeof(strDropped), "Drops: %d (%.1f%%)", (int)dropped_frames, drop_percent);

            msg_->setText(QString("%1 | %2 | %3 | %4")
                .arg(strDuration)
                .arg(strBps.c_str())
                .arg(strFps)
                .arg(strDropped));
            msg_->setToolTip(QString("Tempo: %1\nBitrate: %2\nFPS: %3\nQuadros perdidos: %4")
                .arg(strDuration)
                .arg(strBps.c_str())
                .arg(strFps)
                .arg(strDropped));
